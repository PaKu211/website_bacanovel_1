...
def write_novel_index(novel_dir, meta_data, novel_title, novel_slug, chapter_count, rate=0):
    ...
    if os.path.exists(index_path):
        try:
            ...
            print(f"  Warning: Could not read existing _index.md — {e}")
            existing = ""
    ...
    try:
        ...
        print(f"  Error: Failed to write _index.md — {e}")
        return
...
def copy_cover(target_folder, novel_slug):
    ...
    try:
        ...
        print(f"  Warning: Failed to copy cover — {e}")
        return False
...
def import_split_novel(target_folder, rate=0.1, force=False):
    ...
    try:
        ...
        print(f"Error: meta.json tidak valid — {e}")
        return False
    ...
    for vol in volumes:
        ...
        ch_files.sort(key=lambda f: int(m[0]) if (m := re.findall(r"\d+", f)) else 0)
    ...
    for idx, (vol, ch_file) in enumerate(chapters, start=1):
        ...
        try:
            ...
            print(f"  Warning: Skipping {ch_path} — {e}")
            skipped += 1
            continue
        ...
        if not write_chapter_file(ch_file_path, ch_title, idx, novel_slug, novel_title, ch_body):
            skipped += 1
            continue
...
def import_single_chapter(args):
    ...
    if args.input and args.input != "-":
        try:
            ...
            print(f"Error: Gagal membaca {args.input} — {e}")
            return False
    else:
    ...
    if not write_chapter_file(ch_file_path, title, chapter_num, novel_slug, args.novel_slug, content):
        return False
...
def main():
    ...
    if os.path.isdir(args.input):
        success = import_split_novel(args.input, rate=args.rate, force=args.force)
    else:
        success = import_single_chapter(args)
